---
timestamp: 'Thu Nov 20 2025 00:04:15 GMT-0500 (Eastern Standard Time)'
parent: '[[..\20251120_000415.743a47fc.md]]'
content_id: e8343d03c328a828e255458fa671da229121218177245f1f85ced12f8bf3211e
---
...
Deno.test("CourseCatalogConcept: defineCourse should successfully define a new course", async () => {
  ...
  console.log("Testing: defineCourse with a valid new course 'Intro to Programming'");
  const courseName = "Intro to Programming";
  const events = [
    { type: "Lecture", times: { days: ["Monday", "Wednesday"], startTime: "10:00", endTime: "11:00" } },
    { type: "Lab", times: { days: ["Friday"], startTime: "13:00", endTime: "15:00" } },
  ];
  ...
  assertEquals(courseInfo[0].events.find(e => e.type === "Lecture")?.times.startTime, "10:00");
  assertEquals(courseInfo[0].events.find(e => e.type === "Lab")?.times.startTime, "13:00");
...
Deno.test("CourseCatalogConcept: defineCourse should fail if startTime is after endTime", async () => {
  ...
  const events = [
    { type: "Lecture", times: { days: ["Monday"], startTime: "11:00", endTime: "10:00" } },
  ];
...
Deno.test("CourseCatalogConcept: defineCourse should fail if startTime is equal to endTime", async () => {
  ...
  const events = [
    { type: "Seminar", times: { days: ["Wednesday"], startTime: "15:00", endTime: "15:00" } },
  ];

  console.log(`Action: defineCourse({ name: "${courseName}", events: [...] }) with equal times`);
  const result = await catalog.defineCourse({ name: courseName, events });
  console.log("Result:", result);

  assertExists((result as { error: string }).error, "Expected an error for invalid meeting time");
  assertEquals((result as { error: string }).error, "Invalid meeting time: startTime 15:00 must be before endTime 15:00.");
...
Deno.test("CourseCatalogConcept: defineCourse should succeed with an empty list of events", async () => {
  ...
  console.log("Testing: defineCourse with an empty events array for 'Independent Study'");
  const courseName = "Independent Study";
  const events: [] = [];

  const result = await catalog.defineCourse({ name: courseName, events });
  console.log("Result:", result);
  assertNotEquals((result as { error: string }).error, undefined, "Course definition with no events should succeed");
  const { course: courseId } = result as { course: ID };

  console.log("Confirming effects: Verifying course exists and has an empty events array");
  const courseInfo = await catalog._getCourseInfo({ courses: [courseId] });
  assertEquals(courseInfo.length, 1);
  assertEquals(courseInfo[0].name, courseName);
...
Deno.test("CourseCatalogConcept: _getAllCourses should return an empty list when no courses exist", async () => {
  ...
  assertEquals(courses.length, 0, "Should return an empty array when no courses are defined");
...
Deno.test("CourseCatalogConcept: _getCourseInfo should return information for specified courses", async () => {
  ...
  const result = await catalog.defineCourse({ name: "Query Course A", events: [{ type: "Lab", times: { days: ["Wednesday"], startTime: "09:00", endTime: "12:00" } }] });
  const { course: courseId } = result as { course: ID };
...
Deno.test("CourseCatalogConcept: _getCourseInfo should return an empty list for non-existent courses", async () => {
  ...
  assertEquals(info.length, 0, "Should return an empty array for a non-existent ID");
...
Deno.test("CourseCatalogConcept: _getCourseInfo should only return info for existing courses when given a mix of IDs", async () => {
  ...
  const result = await catalog.defineCourse({ name: "Existing Course", events: [] });
  const { course: existingId } = result as { course: ID };
  const fakeId = "course:does-not-exist" as ID;

  console.log("Testing query: _getCourseInfo with one existing and one non-existent ID");
  const info = await catalog._getCourseInfo({ courses: [existingId, fakeId] });
  console.log("Result:", info);

  assertEquals(info.length, 1, "Should only return info for the one existing course");
...
Deno.test("CourseCatalogConcept: _getEventTimes should return meeting times for specified events", async () => {
  ...
  const courseResult = await catalog.defineCourse({ name: "Event Test Course", events: [{ type: "Lecture", times: { days: ["Friday"], startTime: "15:00", endTime: "16:30" } }] });
...
Deno.test("CourseCatalogConcept: _getEventTimes should return an empty list for non-existent events", async () => {
  ...
  assertEquals(times.length, 0, "Should return an empty array for a non-existent event ID");
