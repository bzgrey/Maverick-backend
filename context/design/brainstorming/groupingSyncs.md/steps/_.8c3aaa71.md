---
timestamp: 'Tue Nov 18 2025 22:01:51 GMT-0500 (Eastern Standard Time)'
parent: '[[../20251118_220151.2ad87031.md]]'
content_id: 8c3aaa7197e4dded8d5bdc156bd04195103294deffba5ae8c93605c4019037ad
---
...
export const NotifyWhenReachTen: Sync = ({ count, user }) => ({
    ...
    where: async (frames) => {
        // The `in Counter: count > 10` clause from the sync specification
        // translates to querying the Counter concept to bind the 'count' variable.
...
If you need to formulate a more complex object, you can specify that in the `where` clause in TypeScript, for example, by constructing a JSON object and binding it to a variable within the frames:

```typescript
// Assuming 'username' is already bound in frames from a previous query
export const ExampleComplexResponse: Sync = ({ request, username, body }) => ({
  when: actions(
    [Requesting.request, { path: "/user-info" }, { request }],
  ),
  where: async (frames) => {
    frames = await frames.query(User._getUsername, { user: frames[0][user] }, { username }); // Example: get username
    return frames.map((frame) => {
      const constructedBody = { "name": frame[username] };
      return { ...frame, [body]: constructedBody }; // Add the 'body' variable to the frame
    });
  },
  then: actions(
