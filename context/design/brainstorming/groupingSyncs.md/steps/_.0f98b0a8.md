...
content_id: 0f98b0a8fb6fbab894047205d4a844c2371526dc30ba4eb6eb608c35e6e559da
---
...
import { Requesting, Sessioning, FileUploading } from "@concepts"; // Removed Sharing as it's not used here

export const ListMyFilesRequest: Sync = ({ request, session, user, file, filename, results }) => ({
  when: actions([Requesting.request, { path: "/my-files", session }, { request }]),
  where: async (frames) => {
    // Make sure to grab the original frame to return to the response
    // The 'request' binding is crucial for responding, so we ensure it's preserved.
    const originalRequestFrame = frames.find((f) => f[request]); // Find a frame with the 'request' binding
    if (!originalRequestFrame) {
      // This should ideally not happen if 'when' matched a request, but as a safeguard.
      return new Frames();
    }
    ...
    // Explicitly check if frames are empty after queries
    if (frames.length === 0) {
      // Then create a new frame with the original request and an empty array for results
      const responseFrame = { ...originalRequestFrame, [results]: [] };
...
1. Grabbing the original frame (or relevant parts of it, like the `request` binding), as the `then` clause needs other bindings.
