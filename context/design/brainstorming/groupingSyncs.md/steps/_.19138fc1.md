...
content_id: 19138fc137749cb759f61056b167a18e801aedc6adfd980d94aa4c27e5ddddb1
---
...
export const PostCommentDeletion: Sync = ({ post, comment, request }) => ({
	...
	where: async (frames) => {
		return await frames
			.query(Comment._getByTarget, { target: post }, { comment }); // Removed trailing comma
	},
	then: actions(
		[Comment.delete, { comment }]
	),
...
```sync
// INCORRECT: This tries to match on both 'question' and 'error' being present in the output, which are mutually exclusive.
sync AddQuestionResponse
when
  Requesting.request (path: "/LikertSurvey/addQuestion") : (request)
...
**Important:** The `then` clause (and `when` clause's input patterns) expects a flat list of *parameters* corresponding to the action's specification, not a single nested object literal as an argument. For example:
...
```typescript
import { Frames } from "@engine"; // Ensure Frames is imported for this example

const name = Symbol("name");
const age = Symbol("age");
const city = Symbol("city");
const scores = Symbol("scores");
