---
timestamp: 'Tue Nov 18 2025 22:12:56 GMT-0500 (Eastern Standard Time)'
parent: '[[../20251118_221256.62e5da79.md]]'
content_id: 51d57b397d61eb13e45fafd1d57a34547e06cf61fa96d2f6b55fda2858b7332c
---
...
where
	...
	in User: _getUsername (user: member) gets memberName: string
	// Collect all members and their names into a single `results` array, grouped by request.
	// Each item in `results` will be an object like: { member: { id: string }, memberName: string }
	results is frames.collectAs([member, memberName], results)
then
	Requesting.respond (request, results: array< { member: { id: string }, memberName: string } >)
...
where
	...
	in User: _getUsername (user: groupOwner) gets ownerName: string
	// Collect all group details into a single `results` array.
	// Each item in `results` will be an object like: { group: { id: string }, groupName: string, groupOwner: { id: string }, ownerName: string }
	results is frames.collectAs([group, groupName, groupOwner, ownerName], results)
then
	Requesting.respond (request, results: array< { group: { id: string }, groupName: string, groupOwner: { id: string }, ownerName: string } >)
